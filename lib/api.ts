...
export const fetchPricesData = async (cabang: string, lingkup: string) => {
    ...
    const data = await res.json();
    if (!res.ok) {
        throw new Error(data?.message || data?.error || `Gagal mengambil data harga (${res.status}).`);
    }
...
export const fetchUserCabangList = async (
    filters?: { cabang?: string; jabatan?: string; search?: string; email_sat?: string; nama_pt?: string; include_branch_scope?: boolean; workspace?: "store" | "dc" }
...
export const regenerateAndDownloadOpnameFinalPdf = async (id: number): Promise<boolean> => {
    const res = await apiFetch(`${API_URL.replace(/\/$/, "")}/api/final_opname/${id}/pdf/regenerate`, {
        method: "POST"
    });
    if (res.status === 404) throw new Error(`Data Opname dengan ID ${id} tidak ditemukan.`);
    if (!res.ok) {
...
export const viewGeneratedPdfOnline = async (
    id: number,
    tipe: "RAB" | "SPK" | "OPNAME" | "OPNAME_FINAL" | "INSTRUKSI_LAPANGAN" | "PROJECT_PLANNING" | "BERKAS_SERAH_TERIMA" | "DOKUMENTASI_BANGUNAN" | "PENGAWASAN" | "PERTAMBAHAN_SPK"
): Promise<boolean> => {
