...
const KATEGORI_CONFIG: Record<DokumenKategori, {
    ...
    OPNAME: {
        ...
        description: 'Daftar dokumen Opname Parsial.',
    },
    OPNAME_FINAL: {
...
const normalizeOpnameDocs = (items: any[], tipe: 'OPNAME' | 'OPNAME_FINAL'): NormalizedDoc[] =>
    items.map(o => ({
        id: o.id,
...
export default function DaftarDokumenPage() {
        ...
        try {
            ...
            if (doc.tipe === 'INTERVENSI') {
                ...
                detail = {
                    ...
                    nomor_spk_sipil: d.nomor_spk_sipil || d.spk?.nomor_spk_sipil,
                    nomor_spk_me: d.nomor_spk_me || d.spk?.nomor_spk_me,
    ...
    const handleViewPDFOnline = useCallback(async (detail: NormalizedDetail) => {
        try {
            if (detail.tipe === 'PENGAWASAN' && detail.is_pdf_pending && detail.link_pdf) {
                                    ...
                                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                                        <div className="px-6 py-4 border-b border-slate-100">
                                            <h4 className="text-sm font-bold text-slate-700 flex items-center gap-2">
                                                <div className={`w-1.5 h-5 ${selectedDetail.tipe === 'OPNAME' ? 'bg-sky-500' : selectedDetail.tipe === 'OPNAME_FINAL' ? 'bg-orange-500' : selectedDetail.tipe === 'INSTRUKSI_LAPANGAN' ? 'bg-pink-500' : 'bg-blue-500'} rounded-full`} />
