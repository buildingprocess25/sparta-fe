      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-6 md:px-6">
        <div className="mb-5 rounded-lg border border-slate-200 bg-white px-5 py-4 shadow-sm">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-red-600">Kontrol akses aplikasi</p>
              <h1 className="mt-2 text-xl font-bold tracking-normal text-slate-950">Pemeliharaan & Jadwal Akses</h1>
            ...
            <Button
              variant="outline"
              onClick={loadStatus}
              disabled={loading || savingMaintenance || savingSchedule}
        ...
        <div className="grid gap-5">
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-start gap-3 mb-6">
              <div className={`flex h-10 w-10 items-center justify-center rounded-lg border ${isMaintenanceActive ? "border-red-200 bg-red-50 text-red-700" : "border-emerald-200 bg-emerald-50 text-emerald-700"}`}>
                {isMaintenanceActive ? <LockKeyhole className="h-5 w-5" /> : <ShieldCheck className="h-5 w-5" />}
              </div>
              <div className="flex-1">
                <Badge className={`rounded-md border px-2.5 py-1 text-[11px] font-bold ${isMaintenanceActive ? "border-red-200 bg-red-50 text-red-700" : "border-emerald-200 bg-emerald-50 text-emerald-700"}`}>
                  {isMaintenanceActive ? "AKTIF" : "NORMAL"}
                </Badge>
                <h2 className="mt-3 text-base font-bold text-slate-950">Mode Pemeliharaan Sistem</h2>
            ...
            <div className="grid gap-3 rounded-lg border border-slate-200 bg-slate-50 p-4 mb-6">
              <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-slate-400">Template pesan pengguna</p>
              <div>
                <p className="text-sm font-bold text-slate-900">{maintenanceStatus?.title || "Sistem sedang dalam pemeliharaan"}</p>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <Button
                ...
                className="h-11 rounded-lg bg-red-700 px-5 font-bold text-white hover:bg-red-800"
              >
              ...
              <Button
                ...
                variant="outline"
                className="h-11 rounded-lg border-emerald-200 bg-white px-5 font-bold text-emerald-700 hover:bg-emerald-50"
              >
            ...
            <div className="mt-6 grid gap-3 rounded-lg border border-slate-200 bg-slate-50 p-4">
              <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-slate-400">Informasi status</p>
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-start gap-3 mb-6">
              ...
              <div className="flex-1">
                <h2 className="text-base font-bold text-slate-950">Jadwal Akses Aplikasi</h2>
                <p className="mt-1 text-sm leading-6 text-slate-500">
            ...
            <div className="space-y-5">
              ...
              {scheduleEnabled && (
                <>
                  <div className="grid gap-4 rounded-lg border border-slate-200 bg-slate-50 p-4">
                    <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-slate-400">Hari akses</p>
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                      <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-slate-400 mb-4">Jam akses user umum</p>
                      <div className="grid gap-3">
                        <div>
                          <Label htmlFor="general-start" className="text-xs font-bold text-slate-900">Jam Mulai</Label>
                          <Input id="general-start" type="time" value={generalStartTime} onChange={(e) => setGeneralStartTime(e.target.value)} disabled={loading || savingSchedule} className="mt-1 rounded-lg bg-white" />
                        </div>
                        <div>
                          <Label htmlFor="general-end" className="text-xs font-bold text-slate-900">Jam Selesai</Label>
                          <Input id="general-end" type="time" value={generalEndTime} onChange={(e) => setGeneralEndTime(e.target.value)} disabled={loading || savingSchedule} className="mt-1 rounded-lg bg-white" />
                        </div>
                      </div>
                    </div>

                    <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                      <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-slate-400 mb-4">Jam akses kontraktor</p>
                      <div className="grid gap-3">
                        <div>
                          <Label htmlFor="contractor-start" className="text-xs font-bold text-slate-900">Jam Mulai</Label>
                          <Input id="contractor-start" type="time" value={contractorStartTime} onChange={(e) => setContractorStartTime(e.target.value)} disabled={loading || savingSchedule} className="mt-1 rounded-lg bg-white" />
                        </div>
                        <div>
                          <Label htmlFor="contractor-end" className="text-xs font-bold text-slate-900">Jam Selesai</Label>
                          <Input id="contractor-end" type="time" value={contractorEndTime} onChange={(e) => setContractorEndTime(e.target.value)} disabled={loading || savingSchedule} className="mt-1 rounded-lg bg-white" />
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-slate-400 mb-3">Informasi jadwal</p>
                <dl className="grid gap-3 text-sm sm:grid-cols-2">
                  <div>
                    <dt className="text-xs font-bold text-slate-400">Terakhir diperbarui</dt>
