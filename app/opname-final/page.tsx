...
function PICOpnameView({ userInfo }: { userInfo: { name: string; role: string; cabang: string; email: string; isSuperHuman?: boolean } }) {
    ...
    const handleSelectRab = async (rabId: string) => {
        ...
        try {
            ...
            try {
                const opnameRes = await fetchOpnameList({ id_toko: rab.id_toko, tipe_opname: 'OPNAME_FINAL' });
            ...
            try {
                const finalRes = await fetchOpnameFinalList({ id_toko: rab.id_toko, aksi: 'terkunci', tipe_opname: 'OPNAME_FINAL' });
    ...
    const handleSubmitItem = async (rabItem: RABDetailItem) => {
        ...
        try {
            ...
            if (input.file) {
                ...
                formData.append('tipe_opname', 'OPNAME_FINAL');
            ...
            try {
                const opnameRes = await fetchOpnameList({ id_toko: selectedRab.id_toko, tipe_opname: 'OPNAME_FINAL' });
...
function KontraktorOpnameView({ userInfo }: { userInfo: { name: string; role: string; cabang: string; email: string; nama_pt?: string } }) {
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            .then(([res, instruksiRes]) => {
                ...
                return fetchOpnameList({ tipe_opname: 'OPNAME_FINAL' }).then(opnameRes => ({ opnameRes, filteredRab, ilTokoIds: new Set(nextIlTokoMap.keys()) }));
            })
    ...
    const handleSelectToko = async (tokoId: number) => {
        ...
        try {
            ...
            const opnameRes = await fetchOpnameList({ id_toko: tokoId, tipe_opname: 'OPNAME_FINAL' });
    ...
    const refreshData = async () => {
        ...
        setIsLoadingItems(true);
        try {
            const opnameRes = await fetchOpnameList({ id_toko: selectedToko.id_toko, tipe_opname: 'OPNAME_FINAL' });
                ...
                <Card className="shadow-sm border-slate-200 relative z-10">
                    ...
                    <div className="p-6 border-b border-slate-100 bg-white rounded-t-xl flex items-center gap-3">
                        ...
                        <div className="flex-1">
                            <h2 className="text-xl font-bold text-slate-800">Review Opname Final</h2>
...
export default function OpnamePage() {
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans pb-12">
            <AppNavbar
                title={appMode === 'pic' ? 'OPNAME FINAL' : 'REVIEW OPNAME FINAL'}
                showBackButton
                backHref="/dashboard"
            />
